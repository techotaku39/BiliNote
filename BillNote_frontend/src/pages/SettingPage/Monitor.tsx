      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">实时监控系统各组件运行状态</p>
          </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              {status && (
                <StatusBadge
                  ...
                  label={status.cuda.available ? '已启用' : '未启用'}
                />
              )}
            </CardHeader>
            <CardContent>
              {loading && !status ? (
                ...
                <div className="space-y-2 text-sm">
                  {status.cuda.available ? (
                    ...
                    <div className="text-muted-foreground">CUDA 不可用，将使用 CPU 模式</div>
                  )}
                </div>
              ) : null}
            </CardContent>
              ...
              {status &&
                (() => {
                  const isLocal =
                    ...
                    status.whisper.transcriber_type === 'mlx-whisper'
                  if (!isLocal) return <StatusBadge ok={true} label="在线引擎" />
                  return (
                    <StatusBadge
                      ok={status.whisper.downloaded}
                      label={status.whisper.downloaded ? '已下载' : '未下载'}
                    />
                  )
                })()}
            </CardHeader>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              {status && (
                <StatusBadge
                  ...
                  label={status.ffmpeg.available ? '可用' : '不可用'}
                />
              )}
            </CardHeader>
            <CardContent>
        ...
        {/* Footer Info */}
        <div className="mt-8 text-center text-xs text-gray-400">状态每 30 秒自动刷新</div>
      </div>
    </ScrollArea>
  )
}
